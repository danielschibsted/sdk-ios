...
/**
 ...
 1. The first update is published to the old account with SPiDTokenStorageModeReadKeychainWriteAll
    (and starts writing tokens to NSUserDefaults)
 2. The second update is published to the new account with SPiDTokenStorageModeReadAllWriteKeychain
    (and starts reading tokens from NSUserDefaults)
 Eventually a subsequent update can revert back to use SPiDTokenStorageModeKeychainOnly
 */
typedef NS_ENUM(NSUInteger, SPiDTokenStorageMode) {
    /** Use only keychain to store and retrieve user tokens */
    SPiDTokenStorageModeKeychainOnly,
    /** Retrieve user tokens from keychain, store to both keychain and NSUserDefaults */
    SPiDTokenStorageModeReadKeychainWriteAll,
    /** Try retrieving user tokens from NSUserDefaults after keychain, store to keychain only */
    SPiDTokenStorageModeReadAllWriteKeychain,
};
